...
KTView::KTView(QWidget *parent) :
	QWidget(parent), listView(0), currentCTCron(0), currentIsTask(true), currentCTTask(0), currentCTVariable(0), clipboardIsTask(true), clipboardCTTask(0), clipboardCTVariable(0) {
	...
	connect(this, SIGNAL(enableRunNow(bool
			)), parent, 
	SLOT(slotEnableRunNow(bool)));

	connect(this, SIGNAL(enableEnabled(bool
			)), parent, 
...
void KTView::refresh() {
	...
	foreach(CTCron* ctCron, const_cast<CTHost&>(ctHost).cron) {
		...
		foreach(CTVariable* ctVariable, ctCron->variable) {
			new KTListVar(variables, ctCron, ctVariable);
		}

		foreach(CTTask* ctTask, ctCron->task) {
			new KTListTask(tasks, ctCron, ctTask);
		}
	}

	resizeEvent();

	connect(listView, SIGNAL(contextMenuRequested(Q3ListViewItem*, const QPoint&, int)), SLOT(slotMenu(Q3ListViewItem*, const QPoint&, int)));

	connect(listView, SIGNAL(selectionChanged(Q3ListViewItem*)), SLOT(slotSetCurrent(Q3ListViewItem*)));

	connect(listView, SIGNAL(doubleClicked(Q3ListViewItem*)), SLOT(slotEdit(Q3ListViewItem*)));

	connect(listView, SIGNAL(returnPressed(Q3ListViewItem*)), SLOT(slotEdit(Q3ListViewItem*)));
...
void KTView::remove() {
	if (currentIsTask) {
		if (currentCTTask) {
			...
			foreach(CTTask* ctTask, currentCTCron->task) {
				if (ctTask == currentCTTask) {
					currentCTCron->task.removeAll(ctTask);
		...
		if (currentCTVariable) {
			foreach(CTVariable* ctVariable, currentCTCron->variable) {
				if (ctVariable == currentCTVariable) {
					currentCTCron->variable.removeAll(ctVariable);
					delete currentCTVariable;
					currentCTVariable = NULL;
					break;
				}
			}
		}
...
QString KTView::absolute() const {
	QString fullCommand = currentCTTask->command;
	...
	QString path = currentCTCron->path();
	int begin(0);
	int end(0);
	QString prefix;
	QString full;
...
emit		(enableModificationButtons(false));
		...
		if (parent->text(0) == KTListVars::getDescription()) {
			...
			emit(enableModificationButtons(true));
			emit(enableRunNow((currentCTTask->enabled) && (!absolute().isEmpty())));
